...
if __name__ == '__main__':
    ...
    rfm = Models(RandomForestClassifier(n_estimators=300, max_depth=6))
    svm = Models(SVC())
    ...
    start_knn = time.time()
    X_train_knn = pca.fit_transform(X_pca_train)
    X_test_knn = pca.transform(X_pca_test)
    ei_knn = knn.EIModel(X_train_knn, X_test_knn, y_train, y_test)
    ...
    plt.xticks(indices + 3 * bar_width, x_labels)
    plt.xlabel('Results')
